...
function K_eigmin(Gi::AbstractArray{T,3},Hi::AbstractArray{T,3},V0::AbstractArray{T,3},Amid_tensor::AbstractArray{T,4};it_solver=false::Bool,itslv_thresh=256::Int64,maxiter=200::Int64,tol=1e-6::Float64) where T<:Number
	...
	if it_solver || prod(K_dims) > itslv_thresh
		...
		function K_matfree(V::AbstractArray{Complex{S},1};Gi=Gtemp::AbstractArray{S,3},Hi=Htemp::AbstractArray{S,3},K_dims=K_dims::NTuple{3,Int},Amid_tensor=Amid_tensor::AbstractArray{S,4},Vout=Vout::AbstractArray{Complex{S},1}) where S<:Number
			...
			return 0.5*Vout::AbstractArray{Complex{S},1}
		end
		r = eigsolve(LinearMap{Complex{T}}(K_matfree,prod(K_dims);ishermitian = true),copy(complex.(V0[:])),1,:SR,ishermitian=true,tol=tol,maxiter=maxiter)
