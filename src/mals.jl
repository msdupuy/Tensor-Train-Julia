...
function init_H_mals(x_tt::TTvector{T},A::TToperator{T},rmax::Int) where {T<:Number}
	...
	H[d-1] = reshape(permutedims(A.cores[d],[3,1,2,4]), :, x_tt.dims[d], 1, x_tt.dims[d], 1) #size(R^A_d, n_d, r_{d+1}, n_d, r_{d+1})
	for i = (d-1) : -1 : 2
		# Update H[i-1]
		rmax_i = min(rmax,prod(x_tt.dims[1:i]),prod(x_tt.dims[i+1:end]))
		H[i-1] = zeros(T,A.rks[i],x_tt.dims[i],rmax_i,x_tt.dims[i],rmax_i)
		Hi = @view(H[i][:,:,1:x_tt.rks[i+2],:,1:x_tt.rks[i+2]])
		Him = @view(H[i-1][:,:,1:x_tt.rks[i+1],:,1:x_tt.rks[i+1]])
...
function init_Hb_mals(x_tt::TTvector{T},b::TTvector{T},rmax::Int) where {T<:Number}
	...
	Hb[d-1] = reshape(permutedims(b.cores[d],[2,1,3]),b.rks[d],b.dims[d],1)
	for i in d-1:-1:2
		rmax_i = min(rmax,prod(x_tt.dims[1:i]),prod(x_tt.dims[i+1:end]))
		Hb[i-1] = zeros(T,b.rks[i+1],b.dims[i+1],rmax_i)
		Hbi = @view(Hb[i][:,:,1:x_tt.rks[i+2]])
		Hbim = @view(Hb[i-1][:,:,1:x_tt.rks[i+1]])
...
function left_core_move_mals(xtt::TTvector{T},i::Integer,V::Array{T,4},tol::Float64,rmax::Integer) where {T<:Number}
	...
	xtt.rks[i+1] = min(length(s_trunc),rmax)
	println("Rank: $(xtt.rks[i+1]),	Max rank=$rmax")
	println("Discarded weight: $((norm(s_V)-norm(s_V[1:xtt.rks[i+1]]))/norm(s_V))")

	# xtt.cores[i+1] = truncated Transpose(v_V)
	xtt.cores[i+1] = permutedims(reshape(v_V'[1:xtt.rks[i+1],:],xtt.rks[i+1],size(V,3),size(V,4)),[2 1 3])
	# xtt.cores[i] = truncated u_V * Diagonal(s_V)
	xtt.cores[i] = reshape(u_V[:, 1:xtt.rks[i+1]] * Diagonal(s_trunc[1:xtt.rks[i+1]]),size(V,1),size(V,2),:)
	xtt.ot[i+1] = 1
	xtt.ot[i] = 0
	return xtt
end
...
function right_core_move_mals(xtt::TTvector{T},i::Integer,V::Array{T,4},tol::Float64,rmax::Integer) where {T<:Number}
	...
	# xtt.cores[i] = truncated u_V
	xtt.cores[i] = reshape(u_V[:, 1:xtt.rks[i+1]], size(V,1), size(V,2), xtt.rks[i+1])
	xtt.ot[i] = -1

	# xtt.cores[i+1] = truncated Diagonal(s_V) * Transpose(v_V)
	xtt.cores[i+1] = permutedims(reshape(Diagonal(s_trunc[1:xtt.rks[i+1]]) * v_V'[1:xtt.rks[i+1],:],xtt.rks[i+1],size(V,3),size(V,4)), [2 1 3])
	xtt.ot[i+1] = 0
	return xtt
end
...
"""
function mals_linsolv(A :: TToperator{T}, b :: TTvector{T}, tt_start :: TTvector{T}; tol=1e-12::Float64,rmax=round(Int,sqrt(prod(tt_start.dims)))) where {T<:Number}
	...
	#while 1==1 #TODO make it work for real
		# First half sweep
		for i = 1:(d-1)
			Gi = @view(G[i][:,1:tt_opt.rks[i],:,1:tt_opt.rks[i],:])
			Hi = @view(H[i][:,:,1:tt_opt.rks[i+2],:,1:tt_opt.rks[i+2]])
			G_bi = @view(G_b[i][:,1:tt_opt.rks[i],:])
			H_bi = @view(H_b[i][:,:,1:tt_opt.rks[i+2]])
			...
			Gip = @view(G[i+1][:,1:tt_opt.rks[i+1],:,1:tt_opt.rks[i+1],:])
		...
		for i = d-1:(-1):1
			...
			if i > 1
				Him = @view(H[i-1][:,:,1:tt_opt.rks[i+1],:,1:tt_opt.rks[i+1]])
				updateH_mals!(tt_opt.cores[i+1], A.cores[i], Hi, Him)
				H_bim = @view(H_b[i-1][:,:,1:tt_opt.rks[i+1]])
...
"""
function mals_eigsolv(A :: TToperator{T}, tt_start :: TTvector{T}; tol=1e-12::Float64,sweep_schedule=[2]::Array{Int64,1},rmax_schedule=[round(Int,sqrt(prod(tt_start.dims)))]::Array{Int64,1},it_solver=false::Bool,linsolv_maxiter=200::Int64,linsolv_tol=max(sqrt(tol),1e-8)::Float64,itslv_thresh=256::Int) where {T<:Number}
	...
	for i in 1:d
		rmax_i = min(rmax,prod(dims[1:i-1]),prod(dims[i:end]))
		G[i] = zeros(dims[i],rmax_i,dims[i],rmax_i,A.rks[i+1])
	end
	G[1][:,1:1,:,1:1,:] = reshape(A.cores[1][:,:,1,:], dims[1],1,dims[1], 1, :)
	...
	while i_schedule <= length(sweep_schedule) 
		...
		for i = 1:(d-1)
			...
			λ,V = K_eigmin_mals(G[i],H[i],tt_opt.cores[i],tt_opt.cores[i+1];it_solver=it_solver,maxiter=linsolv_maxiter,tol=linsolv_tol)
			println("Eigenvalue: $λ")
			E = vcat(E,λ)
			tt_opt = right_core_move_mals(tt_opt,i,V,tol,rmax_schedule[i_schedule])
			r_hist = vcat(r_hist,maximum(tt_opt.rks))
			# Update G[i+1],G_b[i+1]
			Gi = @view(G[i][:,1:tt_opt.rks[i],:,1:tt_opt.rks[i],:])
			Gip = @view(G[i+1][:,1:tt_opt.rks[i+1],:,1:tt_opt.rks[i+1],:])
			update_G!(tt_opt.cores[i],A.cores[i+1],Gi,Gip)
