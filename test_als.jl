...
function test_als_eig_schedule()
    n = 10
    L = Lap(n,3)
    E = eigmin(L)
    x0 = ones(n,n,n)
...
function test_mals()
    ...
    x0_tt = ttv_decomp(x0,1)
    x = L\b[:]
    x_tt = mals(L_tt,b_tt,x0_tt;tol=0.0)
    y = ttv_to_tensor(x_tt)
    @test isapprox(y[:],x)
end

function test_mals_eig()
    n = 10
    L = Lap(n,3)
    E = eigmin(L)
    x0 = randn(n,n,n)
    L_tt = tto_decomp(reshape(L,n,n,n,n,n,n),1)
    x0_tt = ttv_decomp(x0,1)
    E_tt,x_tt,r_hist = mals_eig(L_tt,x0_tt;tol=0.0)
    y = ttv_to_tensor(x_tt)
    @test isapprox(E,E_tt[end])
    E_tt,x_tt,r_hist = mals_eig(L_tt,x0_tt,it_solver=true)
    @test isapprox(E,E_tt[end],atol=1e-8)
end
