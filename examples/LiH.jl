...
function lih(;n_cas = 10, rmax=64,ε=1e-5,ε_tt=1e-5,fiedler=false,history=true,print=true)
  ...
  E,ψ_tt,r_hist =dmrg_eigsolv(H_tto,v;sweep_schedule=[4,8,12,16],rmax_schedule=[32,64,96,128],it_solver=true) 
  if print
    date = now()
  ...
  if fiedler
    v = slater(F[3],2n_cas)
    v = tt_up_rks(v,64;ϵ_wn=1e-2)
    E,ψ_tt,r_hist =dmrg_eigsolv(H_tto,v;sweep_schedule=[4],rmax_schedule=[48]) 
